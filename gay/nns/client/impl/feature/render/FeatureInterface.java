		...
		for (AbstractFeature feature : featureList) {
			if (feature.isEnabled()) {
				...
				switch (arraylistMode.toLowerCase()) {
					case "left" -> Gui.drawRect((float) (sr.getScaledWidth() - fr.getStringWidth(arraylist) - offset - 2), (float) (y - 1), (float) (sr.getScaledWidth() - fr.getStringWidth(arraylist) - offset - 1), (float) (y + fr.FONT_HEIGHT + spacing - 1), getColor(feature, (int) y * 10).getRGB());
					case "right" -> Gui.drawRect((float) (sr.getScaledWidth() - offset + 1), (float) (y - 1), (float) ((float) sr.getScaledWidth() - offset + 2), (float) (y + fr.FONT_HEIGHT + spacing - 1), getColor(feature, (int) y * 10).getRGB());
					case "top" -> {
